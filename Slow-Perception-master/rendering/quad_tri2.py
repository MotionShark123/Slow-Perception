...
class drawer:
    def __init__(self):
        ...
        self.json_caption = '''As shown in the figure, {label1}{label2}{label3}{label4} is a {shape}, points {label5} and {label6} are on {label2}{label3} and {label3}{label4} respectively, connecting {label2}{label4}, {label1}{label5}, {label1}{label6}, {label5}{label6}, point {label7} is on {label5}{label6}, connecting {label1}{label7}.'''
...
\\draw [line width={width}pt{dashed}, {color1}] ({x1}, {y1}) -- ({x5}, {y5});
\\draw [line width={width}pt{dashed}, {color1}] ({x1}, {y1}) -- ({x6}, {y6});
\\draw [line width={width}pt{dashed}, {color1}] ({x1}, {y1}) -- ({x7}, {y7});
\\draw [line width={width}pt{dashed}, {color1}] ({x5}, {y5}) -- ({x6}, {y6});
\\draw [line width={width}pt{dashed}, {color1}] ({x2}, {y2}) -- ({x4}, {y4});
'''
    def get_shape(self):
        return random.choices(self.shapes, weights=[0.05, 0.15, 0.1, 0.1, 0.5, 0.05, 0.05, 0], k=1)[0];
        # return 'Square'
    ...
    def generate_content(self):
        ...
        x5, y5 = i * coordinates[1][0] + (1 - i) * coordinates[2][0], i * coordinates[1][1] + (1 - i) * coordinates[2][1]
        i = random.uniform(0.3, 0.7)
        x6, y6 = i * coordinates[3][0] + (1 - i) * coordinates[2][0], i * coordinates[3][1] + (1 - i) * coordinates[2][1]
        points = coordinates.copy()
        points.append((x5, y5))
        locations.append(self.determine_fifth_point_position(points))
        points = coordinates.copy()
        ...
        i = random.uniform(0.3, 0.7)
        (x7, y7) = i * x5 + (1 - i) * x6, i * y5 + (1 - i) * y6,
        locations.append(self.determine_position_by_angles(LineString([coordinates[0], (x7, y7)])))
...
def process_task(index):
    ...
    while (success < 200):
        ...
        newdict = {
            'id': success,
            'image': f'quad_tri2/{image_fold_name}/page_{success}.png',
            'conversations': [{
                    'from': 'human',
                    'value': '<image>\nOCR with format: '
                }, {
...
if __name__ == '__main__':
    ...
    main(args)
    file_paths = [f'quad_tri2_{i}.json' for i in range(10)]
    merged_data = []
    for file_path in file_paths:
        with open(file_path, 'r') as file:
            data = json.load(file)
            merged_data.extend(data)
    with open('quad_tri2_pre.json', 'w') as output_file:
        json.dump(merged_data, output_file, indent=4)
    os.system('rm -rf curr*')
    input_file = 'quad_tri2_pre.json'
    output_file = 'quad_tri2.json'
    process_json(input_file, output_file)
    os.system('rm -rf quad_tri2_*.json')
