...
from SP.model.vision_encoder.vary_b import build_vary_vit_b
# from SP.model.plug.blip_process import BlipImageEvalProcessor
from SP.model.plug.transforms import train_transform, test_transform
class SPConfig(Qwen2Config):
    model_type = "SP"


class SPQwenModel(Qwen2Model):
    config_class = SPConfig

    def __init__(self, config: Qwen2Config):
        super(SPQwenModel, self).__init__(config)
    ...
    def initialize_vision_modules(
        ...
        # Vary old codes, not use in SP
        image_processor = train_transform
        # 1024*1024
    ...
    def forward(
        ...
        # if True:
            ...
            for cur_input_ids, cur_input_embeds, cur_image_features in zip(input_ids, inputs_embeds, image_features):
                # print((cur_input_ids == im_patch_token).sum())
        ...
        return super(SPQwenModel, self).forward(
...
class SPQwenForCausalLM(Qwen2ForCausalLM):
    config_class = SPConfig
    # supports_gradient_checkpointing = True

    def __init__(self, config):
        super(Qwen2ForCausalLM, self).__init__(config)
        self.model = SPQwenModel(config)
    ...
    #     if isinstance(module, SPQwenModel):
...
AutoConfig.register("SP", SPConfig)
AutoModelForCausalLM.register(SPConfig, SPQwenForCausalLM)
