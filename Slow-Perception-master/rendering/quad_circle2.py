...
\\coordinate ({label5}) at ({x0}, {y0});
\\coordinate ({label6}) at ({x5}, {y5});
\\coordinate ({label7}) at ({x6}, {y6});
\\coordinate ({label8}) at ({x7}, {y7});
\\coordinate ({label9}) at ({x8}, {y8});
{vis1}\\draw [{dashed}, {color1}] ({x0},{y0}) -- ({x5},{y5});
{vis1}\\tkzMarkRightAngle[size=0.2]({label5},{label6},{label1});
{vis2}\\draw [{dashed}, {color1}] ({x0},{y0}) -- ({x6},{y6});
{vis2}\\tkzMarkRightAngle[size=0.2]({label5},{label7},{label2});
{vis3}\\draw [{dashed}, {color1}] ({x0},{y0}) -- ({x7},{y7});
{vis3}\\tkzMarkRightAngle[size=0.2]({label5},{label8},{label3});
{vis4}\\draw [{dashed}, {color1}] ({x0},{y0}) -- ({x8},{y8});
{vis4}\\tkzMarkRightAngle[size=0.2]({label5},{label9},{label4});
...
# '''
    ...
    def determine_position_by_angles(self, line1, line2, line3, line4):
        angle1 = self.get_angle(line1)
        angle2 = self.get_angle(line2)
        angle3 = self.get_angle(line3)
        angle4 = self.get_angle(line4)
        if all((angle < -22.5 or angle >= 22.5) for angle in [angle1, angle2, angle3, angle4]):
            return "right"
        if all((angle < 22.5 or angle >= 67.5) for angle in [angle1, angle2, angle3, angle4]):
            return "above right"
        if all((angle < 67.5 or angle >= 112.5) for angle in [angle1, angle2, angle3, angle4]):
            return "above"
        if all((angle < 112.5 or angle >= 157.5) for angle in [angle1, angle2, angle3, angle4]):
            return "above left"
        if all((angle < 157.5 and angle >= -157.5) for angle in [angle1, angle2, angle3, angle4]):
            return "left"
        if all((angle < -157.5 or angle >= -112.5) for angle in [angle1, angle2, angle3, angle4]):
            return "below left"
        if all((angle < -112.5 or angle >= -67.5) for angle in [angle1, angle2, angle3, angle4]):
            return "below"
        if all((angle < -67.5 or angle >= -22.5) for angle in [angle1, angle2, angle3, angle4]):
            return "below right"
        return "above right"
    ...
    def generate_content(self):
        ...
        location = self.determine_position_by_angles(LineString([(x0, y0), (x5, y5)]),
                                                    LineString([(x0, y0), (x6, y6)]),
                                                    LineString([(x0, y0), (x7, y7)]),
                                                    LineString([(x0, y0), (x8, y8)]))
...
def process_task(index):
    ...
    while (success < 2000):
        ...
        newdict = {
            'id': success,
            'image': f'quad_circle2/{image_fold_name}/page_{success}.png',
            'conversations': [{
                    'from': 'human',
                    'value': '<image>\nOCR with format: '
                }, {
            ...
            'caption': 'quadrilateral with inscribed circle, line connecting center of circle to tangent point'
        }
...
if __name__ == '__main__':
    ...
    main(args)
    file_paths = [f'quad_circle2_{i}.json' for i in range(10)]
    merged_data = []
    for file_path in file_paths:
        with open(file_path, 'r') as file:
            data = json.load(file)
            merged_data.extend(data)
    with open('quad_circle2_pre.json', 'w') as output_file:
        json.dump(merged_data, output_file, indent=4)
    os.system('rm -rf curr*')
    input_file = 'quad_circle2_pre.json'
    output_file = 'quad_circle2.json'
    process_json(input_file, output_file)
    os.system('rm -rf quad_circle2_*.json')
