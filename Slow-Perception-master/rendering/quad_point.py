...
{vis1}\\draw [{dashed}, {color1}] ({x0},{y0}) -- ({x1},{y1});
{vis2}\\draw [{dashed}, {color1}] ({x0},{y0}) -- ({x2},{y2});
{vis3}\\draw [{dashed}, {color1}] ({x0},{y0}) -- ({x3},{y3});
{vis4}\\draw [{dashed}, {color1}] ({x0},{y0}) -- ({x4},{y4});
{vis5}\\draw [{dashed}, {color1}] ({x1},{y1}) -- ({x3},{y3});
{vis6}\\draw [{dashed}, {color1}] ({x2},{y2}) -- ({x4},{y4});
'''
    ...
    def generate_content(self):
        ...
        if vis1 == '%' and vis2 == '%':
            ...
            if vis3 == '%':
                ...
                vis4 = '' if random.uniform(0, 1) > 0.5 else '%'
        vis5 = '' if random.uniform(0, 1) > 0.5 else '%'
        vis6 = '' if random.uniform(0, 1) > 0.5 else '%'
        location = self.determine_position_by_angles(LineString([(x0, y0), (coordinates[0][0], coordinates[0][1])]),
                                                     LineString([(x0, y0), (coordinates[1][0], coordinates[1][1])]),
                                                     LineString([(x0, y0), (coordinates[2][0], coordinates[2][1])]),
                                                     LineString([(x0, y0), (coordinates[3][0], coordinates[3][1])]))
...
def process_task(index):
    ...
    while (success < 2000):
        ...
        newdict = {
            'id': success,
            'image': f'quad_point/{image_fold_name}/page_{success}.png',
            'conversations': [{
                    'from': 'human',
                    'value': '<image>\nOCR with format: '
                }, {
            ...
            'caption': 'quadrilateral and a point'
        }
...
if __name__ == '__main__':
    ...
    main(args)
    file_paths = [f'quad_point_{i}.json' for i in range(10)]
    merged_data = []
    for file_path in file_paths:
        with open(file_path, 'r') as file:
            data = json.load(file)
            merged_data.extend(data)
    with open('quad_point_pre.json', 'w') as output_file:
        json.dump(merged_data, output_file, indent=4)
    os.system('rm -rf curr*')
    input_file = 'quad_point_pre.json'
    output_file = 'quad_point.json'
    process_json(input_file, output_file)
    os.system('rm -rf quad_point_*.json')
