...
{vis_m}\\node [{location_m}, {color_m}] at ({x6},{y6}) {{{middle}}};
{vis_e}\\node [{location_e}, {color_e}] at ({x7},{y7}) {{{end}}};
...
def process_task(index):
    ...
    while (success < 10):
        ...
        newdict = {
            'id': success,
            'image': f'quad_line2/{image_fold_name}/page_{success}.png',
            'conversations': [{
                    'from': 'human',
                    'value': '<image>\n'
                }, {
...
if __name__ == '__main__':
    ...
    main(args)
    file_paths = [f'quad_line2_{i}.json' for i in range(10)]
    merged_data = []
    for file_path in file_paths:
        with open(file_path, 'r') as file:
            data = json.load(file)
            merged_data.extend(data)
    with open('quad_line2.json', 'w') as output_file:
        json.dump(merged_data, output_file, indent=4)
    os.system('rm -rf quad_line2_*.json')
