...
\\node [{color6}, font=\scriptsize] at ({x0}, {y0}) {{{label5}}};
...
def process_task(index):
    ...
    while (success < 2000):
        ...
        newdict = {
            'id': success,
            'image': f'quad_circle1/{image_fold_name}/page_{success}.png',
            'conversations': [{
                    'from': 'human',
                    'value': '<image>\nOCR with format: '
                }, {
            ...
            'caption': 'quadrilateral with inscribed circle'
        }
...
if __name__ == '__main__':
    ...
    main(args)
    file_paths = [f'quad_circle1_{i}.json' for i in range(10)]
    merged_data = []
    for file_path in file_paths:
        with open(file_path, 'r') as file:
            data = json.load(file)
            merged_data.extend(data)
    with open('quad_circle1_pre.json', 'w') as output_file:
        json.dump(merged_data, output_file, indent=4)
    os.system('rm -rf curr*')
    input_file = 'quad_circle1_pre.json'
    output_file = 'quad_circle1.json'
    process_json(input_file, output_file)
    os.system('rm -rf quad_circle1_*.json')
