...
class drawer:
    def __init__(self):
        ...
        self.json_caption = '''As shown in the figure, quadrilateral {label1}{label2}{label3}{label4} is a square, {label4}{label5} is parallel to {label1}{label3}, and straight line {label5}{label3} intersects the extended line of {label4}{label1} at {label6}.'''
...
\\draw [line width={width}pt{dashed}, {color1}] ({x1}, {y1}) -- ({x3}, {y3});
\\draw [line width={width}pt{dashed}, {color1}] ({x4}, {y4}) -- ({x5}, {y5});
\\draw [line width={width}pt{dashed}, {color1}] ({x5}, {y5}) -- ({x6}, {y6}) -- ({x1}, {y1});
'''
    def get_shape(self):
    ...
    def generate_square(self):
        ...
        x4, y4 = x1 + size, y1
        x6, y6 = x1 - random.uniform(0.33 * size, 3 * size), y1
        x5, y5 = x4 + (x4 - x6) / (x1 - x6) * size, y4 - (x4 - x6) / (x1 - x6) * size
    ...
    def rotate_points(self, points, angle_degrees):
        ...
        if random.uniform(0, 1) > 0.5:
            for _ in range(len(rotated_points)):
                rotated_points[_] = (-rotated_points[_][0], rotated_points[_][1])
    ...
    def generate_content(self):
        ...
        locations[0] = self.determine_position_by_angles(LineString([(0, 0), (coordinates[0][0] - coordinates[1][0], coordinates[0][1] - coordinates[1][1])]))
        CB = (coordinates[1][0] - coordinates[2][0], coordinates[1][1] - coordinates[2][1])
        CE = (coordinates[4][0] - coordinates[2][0], coordinates[4][1] - coordinates[2][1])
        angle_bisector = CB / np.linalg.norm(CB) + CE / np.linalg.norm(CE)
        x5, y5 = angle_bisector
        locations[2] = self.determine_position_by_angles(LineString([(0, 0), (x5, y5)]))
        DE = (coordinates[4][0] - coordinates[3][0], coordinates[4][1] - coordinates[3][1])
        CE = (coordinates[4][0] - coordinates[2][0], coordinates[4][1] - coordinates[2][1])
        angle_bisector = DE / np.linalg.norm(DE) + CE / np.linalg.norm(CE)
        x5, y5 = angle_bisector
        locations.append(self.determine_position_by_angles(LineString([(0, 0), (x5, y5)])))
        AF = (coordinates[5][0] - coordinates[0][0], coordinates[5][1] - coordinates[0][1])
        CF = (coordinates[5][0] - coordinates[2][0], coordinates[5][1] - coordinates[2][1])
        angle_bisector = AF / np.linalg.norm(AF) + CF / np.linalg.norm(CF)
        x5, y5 = angle_bisector
        locations.append(self.determine_position_by_angles(LineString([(0, 0), (x5, y5)])))
...
def process_task(index):
    success = 0
    fail = 0
    image_fold = f'./quad_point2/quad_images_{index}'
    image_fold_name = f'quad_images_{index}'
    if not os.path.exists(image_fold):
        os.makedirs(image_fold)
    new_json_file = f'./quad_point2_{index}.json'

    all_list = []
    pdar = tqdm(total=200, desc=f'Curr_process{index} is processing')
    while (success < 200):
        if fail > 200:
        ...
        newdict = {
            'id': success,
            'image': f'quad_point2/{image_fold_name}/page_{success}.png',
            'conversations': [{
                    'from': 'human',
                    'value': '<image>\nOCR with format: '
                }, {
...
if __name__ == '__main__':
    ...
    main(args)
    file_paths = [f'quad_point2_{i}.json' for i in range(10)]
    merged_data = []
    for file_path in file_paths:
        with open(file_path, 'r') as file:
            data = json.load(file)
            merged_data.extend(data)
    with open('quad_point2_pre.json', 'w') as output_file:
        json.dump(merged_data, output_file, indent=4)
    os.system('rm -rf curr*')
    input_file = 'quad_point2_pre.json'
    output_file = 'quad_point2.json'
    process_json(input_file, output_file)
    os.system('rm -rf quad_point2_*.json')
