...
class drawer:
    def __init__(self):
        ...
        self.funcs = {
            'Perpendicular Line': self.func_1
            # 'Fold': self.func_2
        }
...
# '''
    ...
    def func_1(self, labels, coordinates, colors):
        problem_cotent = '''{fill}\\fill[{colorfill}, opacity=0.2] ({x1},{y1}) -- ({x2},{y2}) -- ({x3},{y3}) -- ({x4},{y4}) -- cycle;
\\coordinate ({start}) at ({x5}, {y5});
\\coordinate ({end}) at ({x9}, {y9});
\\coordinate (E) at ({x6}, {y6});
{H_vis}\\node [{location}] at ({x6},{y6}) {{{label5}}};
{line6}\\draw [{dashed}, {color}] ({x5},{y5}) -- ({x6},{y6});
{line7}\\draw [{dashed}, {color}] ({x6},{y6}) -- ({x7},{y7});
{line8}\\draw [{dashed}, {color}] ({x6},{y6}) -- ({x8},{y8});
{angle}\\tkzMarkRightAngle[size=0.2]({start},E,{end});
'''     
        fill = '%' if random.uniform(0, 1) > 0.5 else ''
        colorfill = random.choice([
            "red", "green", "blue", "cyan", "magenta", "yellow", "orange",
            "purple", "brown", "pink", "lime", "olive", "teal", "violet", "gray"
        ])  
        dashed = 'dashed' if random.uniform(0, 1) < 0.9 else 'thick'
        angle = '%' if random.uniform(0, 1) > 0.99 else ''
        if random.uniform(0, 1) < 1:
            # if self.shape == 'Rectangle' or self.shape == 'Square':
            start = random.randint(0, 3)
            if random.uniform(0, 1) < 0.5:
                F, G, H = coordinates[start], coordinates[(start + 1) % 4], coordinates[(start + 2) % 4]
                E = self.projection_point(F, G, H)
                end = labels[(start + 1) % 4] if math.fabs(E[0] - G[0]) + math.fabs(E[1] - G[1]) > 0.02 else labels[(start + 2) % 4]
                start = labels[start]
            else:
                F, G, H = coordinates[start], coordinates[(start + 2) % 4], coordinates[(start + 3) % 4]
                E = self.projection_point(F, G, H)
                end = labels[(start + 2) % 4] if math.fabs(E[0] - G[0]) + math.fabs(E[1] - G[1]) > 0.02 else labels[(start + 3) % 4]
                start = labels[start]
            if math.fabs(E[0] - G[0]) + math.fabs(E[1] - G[1]) > 0.02 and math.fabs(E[0] - H[0]) + math.fabs(E[1] - H[1]) > 0.02:
                H_vis = ''
            else:
                H_vis = '%'
            vec1 = (E[0] - G[0], E[1] - G[1])
            vec2 = (E[0] - H[0], E[1] - H[1])
            (x9, y9) = (H[0], H[1]) if math.fabs(vec1[0]) + math.fabs(vec1[1]) < math.fabs(vec2[0]) + math.fabs(vec2[1]) else (G[0], G[1])
            cross_product = vec1[0] * vec2[0] + vec1[1] * vec2[1]
            if cross_product < -1e-6 or math.fabs(vec1[0]) + math.fabs(vec1[1]) < 1e-6 or math.fabs(vec2[0]) + math.fabs(vec2[1]) < 1e-6:
                line7 =  '%'
                line8 =  '%'
            else:
                vec1 = (G[0] - H[0], G[1] - H[1])
                vec2 = (E[0] - H[0], E[1] - H[1])
                cross_product = vec1[0] * vec2[0] + vec1[1] * vec2[1]
                line7 = '' if cross_product > 1e-6 else '%'
                vec1 = (H[0] - G[0], H[1] - G[1])
                vec2 = (E[0] - G[0], E[1] - G[1])
                cross_product = vec1[0] * vec2[0] + vec1[1] * vec2[1]
                line8 = '' if cross_product > 1e-6 else '%'
            points = coordinates.copy()
            points.append(E)
            location = self.determine_fifth_point_position(points)
            locations = []
            for _ in range(4):
                points = points[1:] + [points[0]];
                locations.append(self.determine_fifth_point_position(points))
            vec1 = (F[0] - G[0], F[1] - G[1])
            vec2 = (F[0] - H[0], F[1] - H[1])
            vec3 = (G[0] - H[0], G[1] - H[1])
            if math.fabs(vec1[0] * vec3[0] + vec1[1] * vec3[1]) < 1e-6 or math.fabs(vec3[0] * vec2[0] + vec3[1] * vec2[1]) < 1e-6:
                line6 = '%'
            else:
                line6 = ''
            return problem_cotent.format(x5=F[0], y5=F[1],
                                         x6=E[0], y6=E[1],
                                         x7=G[0], y7=G[1],
                                         x8=H[0], y8=H[1],
                                         x9=x9, y9=y9,
    ...
    def generate_content(self):
        ...
        if self.type == 'Perpendicular Line':
        ...
        #     locations.append(self.determine_fifth_point_position(points))
        problem_content, locations = self.funcs[self.type](labels, coordinates, colors)
...
def process_task(index):
    ...
    while (success < 2000):
        ...
        newdict = {
            'id': success,
            'image': f'quad_pp/{image_fold_name}/page_{success}.png',
            'conversations': [{
                    'from': 'human',
                    'value': '<image>\n'
                }, {
...
if __name__ == '__main__':
    ...
    main(args)
    file_paths = [f'quad_pp_{i}.json' for i in range(10)]
    merged_data = []
    for file_path in file_paths:
        with open(file_path, 'r') as file:
            data = json.load(file)
            merged_data.extend(data)
    with open('quad_pp_pre.json', 'w') as output_file:
        json.dump(merged_data, output_file, indent=4)
    os.system('rm -rf curr*')
    input_file = 'quad_pp_pre.json'
    output_file = 'quad_pp.json'
    process_json(input_file, output_file)
    os.system('rm -rf quad_pp_*.json')